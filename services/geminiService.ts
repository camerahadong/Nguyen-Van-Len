...
export const initializeSession = (language: OutputLanguage = 'vi') => {
  ...
  const brandName = language === 'vi' ? "Thanh Hà Runner" : "Thanh Ha Runner";
  const systemContext = language === 'vi' 
    ? `Bạn là đại diện của ${brandName} - Câu lạc bộ chạy bộ uy tín. Nhiệm vụ của bạn là hướng dẫn người mới, các vận động viên trong câu lạc bộ Thanh Hà Runner cũng như các vận động viên khác, thực hiện các bước nghiên cứu và viết bài chuyên sâu.`
    : `You are the representative of ${brandName} - A prestigious running club. Your task is to guide beginners, athletes in the Thanh Ha Runner club, as well as other athletes, by executing research and writing steps according to detailed requests.`;
...
const WEBSITE_URL = "https://thr.vn";
...
export const executeStep4 = async (keyword: string, language: OutputLanguage): Promise<string> => {
  ...
  const brandName = language === 'vi' ? "Thanh Hà Runner" : "Thanh Ha Runner";
  const industry = language === 'vi' ? "Chạy bộ, Marathon, Câu lạc bộ Thanh Hà Runner" : "Running, Marathon, Thanh Ha Runner Club";
  const audience = language === 'vi' ? "Người mới tập chạy, vận động viên câu lạc bộ Thanh Hà Runner, và các vận động viên khác" : "Beginner runners, Thanh Ha Runner club athletes, and other athletes";
